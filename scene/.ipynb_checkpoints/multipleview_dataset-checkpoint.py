...
class multipleview_dataset(Dataset):
    ...
    def get_image_number(self, image_name):
        return int(os.path.basename(image_name)[5:-4])
    def load_images_path(self, cam_folder, cam_extrinsics,cam_intrinsics,split):
        ...
        image_poses=[]
        image_times=[]
        sorted_images = sorted(cam_extrinsics.values(), key=lambda extr: self.get_image_number(image_name=extr.name))

        for extr in sorted_images:
